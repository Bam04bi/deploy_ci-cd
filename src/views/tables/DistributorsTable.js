...
// Define the DistributorsTable component
function DistributorsTable() {
  ...
  const [numberOfDistributors, setNumberOfDistributors] = useState(0); // State for the number of distributors
  ...
  const [DIS, setDIS] = useState(); // State to store distributor data
  ...
  const loadBlockchaindata = async () => {
    try {
      ...
      if (networkData) {
        ...
        // Retrieve distributor data from the smart contract
        ...
        // Set distributor data and update the numberOfDistributors state
        setDIS(dis);
